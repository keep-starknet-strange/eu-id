...
pub fn decompose(profile: MlDsaProfile, r: u32) -> (i32, i32) {
...
pub fn high_bits(profile: MlDsaProfile, r: u32) -> i32 {
    decompose(profile, r).0
}

/// FIPS 204 Algorithm 39 `UseHint(h, r)`: recover the high bits of the true
/// commitment given the one-bit hint `h ∈ {0, 1}`.
pub fn use_hint(profile: MlDsaProfile, hint: u8, r: u32) -> i32 {
    let (r1, r0) = decompose(profile, r);
...
pub fn use_hint_poly(profile: MlDsaProfile, hint: &[u8; N], w_approx: &[u32; N]) -> [u32; N] {
    let mut w1 = [0u32; N];
    for i in 0..N {
        w1[i] = use_hint(profile, hint[i], w_approx[i]) as u32;
    }
    w1
}

/// FIPS 204 Algorithm 28 `w1Encode` for the selected profile. It packs each
/// coefficient with the profile's fixed bit width into the `SHAKE-256` input.
pub fn w1_encode(profile: MlDsaProfile, w1: &[[u32; N]]) -> Vec<u8> {
...
mod tests {
    ...
    fn decompose_reconstructs() {
        for &r in &[0u32, 1, GAMMA2, ALPHA, Q - 1, 123_456, 7_777_777] {
            let (r1, r0) = decompose(ML_DSA_65, r);
    ...
    fn use_hint_zero_is_high_bits() {
        for &r in &[0u32, 42, GAMMA2, 999_999] {
            assert_eq!(use_hint(ML_DSA_65, 0, r), high_bits(ML_DSA_65, r));
        }
    }
    ...
    fn w1_encode_packs_two_coeffs_per_byte() {
        ...
        w1[0][0] = 0x0a;
        w1[0][1] = 0x03;
        let bytes = w1_encode(ML_DSA_65, &w1);
    ...
    fn mldsa44_w1_encode_packs_four_six_bit_coefficients_into_three_bytes() {
        ...
        let bytes = w1_encode(ML_DSA_44, &w1);
