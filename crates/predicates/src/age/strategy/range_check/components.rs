...
pub fn components(
    ...
    let day_delta_component = DayDeltaTableComponent::new(
        allocator,
        Preprocessed::day_range()
            .blind_eval(lookup_elements.day_delta)
            .with_claim_mask(claim_mask_beta),
        day_delta_claimed_sum,
    );
    let month_delta_component = MonthDeltaTableComponent::new(
        allocator,
        Preprocessed::month_range()
            .blind_eval(lookup_elements.month_delta)
            .with_claim_mask(claim_mask_beta),
    ...
    let year_delta_component = YearDeltaTableComponent::new(
        ...
        Preprocessed::year_range(&public.bounds)
            .blind_eval(lookup_elements.year_delta)
            .with_claim_mask(claim_mask_beta),
