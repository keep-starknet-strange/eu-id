...
use crate::carrier::{
    build_folded_coefficients, build_fractions, collect_lookups, Fractions, InteractionData,
    LookupKind, N_TOTAL_LOOKUPS,
};
...
fn gkr_input_layer(fracs: Fractions, log_size: u32) -> Layer<SimdBackend> {
    ...
    let (mut numerators, mut denominators) = fracs.into_parts();
    assert!(
        numerators.capacity() >= packed_size,
        "carrier numerator allocation must include the padded GKR slots"
    );
    assert!(
        denominators.capacity() >= packed_size,
        "carrier denominator allocation must include the padded GKR slots"
    );
...
mod tests {
    ...
    /// Former columnar LogUp layout, retained as an independent claim check.
    fn columnar_claimed_sum_reference(fracs: &Fractions, log_size: u32) -> SecureField {
        const BATCH: usize = 4;
        let mut generator = LogupTraceGenerator::new(log_size);
        for first in (0..fracs.n_slots()).step_by(BATCH) {
            let last = (first + BATCH).min(fracs.n_slots());
            let mut column = generator.new_col();
            let (first_numerator, first_denominator) = fracs.slot(first);
            for vector_row in 0..fracs.n_vector_rows() {
                ...
                for slot in first + 1..last {
                    let (next_numerator, next_denominator) = fracs.slot(slot);
        ...
        let run = |workers| {
            ThreadPoolBuilder::new()
                ...
                .install(|| {
                    ...
                    let prover = RoundGkrProver::new(&relations, data.clone());
    ...
    fn replayed_numerators_preserve_every_lookup_family() {
        ...
        const SCHEDULE_SLOT: usize = 0;
        const INPUT_SLOT: usize = 1;
        const FIRST_ARITHMETIC_SLOT: usize = 2;
        const OUTPUT_SLOT: usize = N_TOTAL_LOOKUPS - 1;
        ...
        for slot in 0..N_TOTAL_LOOKUPS {
            let numerators = fracs.slot(slot).0;
            for (row, numerator) in numerators.iter().copied().enumerate() {
                let header = trace[HEADER_COLUMN].values.data[row];
                let round = trace[ROUND_COLUMN].values.data[row];
                let final_round = trace[FINAL_COLUMN].values.data[row];
                let expected = match slot {
                    SCHEDULE_SLOT => header + round,
                    INPUT_SLOT => -header,
                    OUTPUT_SLOT => final_round,
                    FIRST_ARITHMETIC_SLOT..OUTPUT_SLOT => round,
                    _ => unreachable!("all carrier lookup slots are classified"),
                };
                assert_eq!(
                    numerator.to_array(),
                    expected.to_array(),
                    "numerator changed at slot {slot}, packed row {row}"
                );
            }
        }
    }
    ...
    #[test]
    fn logical_source_rotation_preserves_sum_but_changes_row_mle() {
        fn rotate_column(column: &mut ColEval) {
        ...
        for column in rotated.trace_mut() {
        ...
        let delta = relation_channel.draw_secure_felt();
        let eq_ws = eq_weights(
            &(0..LOG_SLOTS)
                .map(|_| relation_channel.draw_secure_felt())
                .collect::<Vec<_>>(),
        );
        let honest_coefficients = build_folded_coefficients(&relations, &data, delta, &eq_ws);
        let shifted_coefficients = build_folded_coefficients(&relations, &rotated, delta, &eq_ws);
        assert_ne!(
            honest_coefficients
                .iter()
                .map(PackedQM31::to_array)
                .collect::<Vec<_>>(),
            shifted_coefficients
                .iter()
                .map(PackedQM31::to_array)
                .collect::<Vec<_>>(),
            "a logical row rotation must change the row-indexed coefficient MLE"
        );
    }

    #[test]
    fn canonical_denominator_allocation_moves_into_gkr_without_copy() {
        const CANONICAL_LOG_SIZE: u32 = 13;
        const EXPECTED_ACTIVE_BYTES: usize = 117_833_728;
        const EXPECTED_PADDED_BYTES: usize = 134_217_728;

        let n_vector_rows = 1usize << (CANONICAL_LOG_SIZE - LOG_N_LANES);
        let active_bytes = N_TOTAL_LOOKUPS * n_vector_rows * std::mem::size_of::<PackedQM31>();
        let padded_bytes =
            N_TOTAL_LOOKUPS.next_power_of_two() * n_vector_rows * std::mem::size_of::<PackedQM31>();
        assert_eq!(active_bytes, EXPECTED_ACTIVE_BYTES);
        assert_eq!(padded_bytes, EXPECTED_PADDED_BYTES);
        ...
        let relations = KeccakRelations::draw(&mut relation_channel);
        let fracs = build_fractions(&relations, &data);
        let denominator_ptr = fracs.denominators().as_ptr();
        assert!(
            fracs.denominator_capacity() >= (1usize << LOG_SLOTS) * fracs.n_vector_rows(),
            "the denominator allocation must reserve all padding"
        );
        let Layer::LogUpMultiplicities {
            numerators,
            denominators,
        } = gkr_input_layer(fracs, log_size)
        else {
            unreachable!("the carrier uses multiplicity leaves")
        };
        assert_eq!(denominators.data.as_ptr(), denominator_ptr);

        let actual_vector_rows = 1usize << (log_size - LOG_N_LANES);
        let active = N_TOTAL_LOOKUPS * actual_vector_rows;
        assert!(numerators.data[active..].iter().all(PackedM31::is_zero));
        assert!(
            denominators.data[active..].iter().all(|value| value
                .to_array()
                .iter()
                .all(|lane| *lane == SecureField::one())),
            "every padded denominator must remain one"
        );
    }
    ...
    fn multiplicities_leaves_and_claimed_sum_match_generic_reference() {
        ...
        assert_eq!(
            multiplicities_sum,
            columnar_claimed_sum_reference(&fracs, log_size),
        ...
        for (name, actual, expected) in [
            ("numerator", &multiplicities_values.0, &generic_values.0),
            ("denominator", &multiplicities_values.1, &generic_values.1),
        ] {
            ...
            {
                panic!(
                    "{name} leaf changed at scalar index {index}: actual={:?}, expected={:?}",
    ...
    fn carrier_denominator_oracle_reconstructs_and_detects_tampering() {
        ...
        let gkr_layer = gkr_input_layer(fractions, log_size);
    ...
    fn multiplicities_layer_preserves_seeded_gkr_and_tieback() {
        ...
        let (prover_blob, prover_tieback, coeff_mle) =
            RoundGkrProver::new(&relations, data.clone()).prove(&mut prover_channel);
    ...
    #[test]
    fn replay_matches_exact_baseline_bytes() {
        ...
        let data = carrier_data(261);
        let mut relation_channel = Blake2sChannel::default();
        let relations = KeccakRelations::draw(&mut relation_channel);
        let fracs = build_fractions(&relations, &data);
        let mut leaf_hash = 0xcbf29ce484222325;
        for value in fracs.numerators() {
            for lane in value.to_array() {
                mix(&mut leaf_hash, &lane.0.to_le_bytes());
            }
        }
        for value in fracs.denominators() {
            for lane in value.to_array() {
                for component in lane.to_m31_array() {
                    mix(&mut leaf_hash, &component.0.to_le_bytes());
                }
            }
        }
        ...
        let claimed_sum = prover.claimed_sum();
        prover_channel.mix_felts(&[claimed_sum]);
        let (blob, _, coeff_mle) = prover.prove(&mut prover_channel);
        let mut proof_hash = 0xcbf29ce484222325;
        mix(&mut proof_hash, &blob);
        let mut coeff_hash = 0xcbf29ce484222325;
        for value in coeff_mle.to_cpu() {
            for component in value.to_m31_array() {
                mix(&mut coeff_hash, &component.0.to_le_bytes());
            }
        }
        assert_eq!(leaf_hash, 0xdd992460e5a14cf5);
        assert_eq!(proof_hash, 0x83141763392699d1);
        assert_eq!(coeff_hash, 0xaab375222211cfa1);
