...
The total envelope length MUST be exactly `46 + P` bytes.
...
It also requires an identity-proof envelope of exactly `46 + P` bytes, where
`P` is the capacity in the verified circuit artifact.
