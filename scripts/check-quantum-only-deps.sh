#!/usr/bin/env bash
#
# Reject classical public-key crypto in normal workspace dependencies.
# Stwo uses `rfc6979` through `starknet-crypto`. The gate permits only that
# parent dependency.
set -euo pipefail
...
echo "==> quantum-only dependency tree is clean"
echo "    (rfc6979 is present only through the stwo engine's starknet-crypto)"
